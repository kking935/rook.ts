...
export const playerDisconnected = (io: any, socket: Socket) => {
	...
	var index = player ? players.indexOf(player) : -1;
	if (index > -1) {
		leaveQueue(socket);
...
export const createMatch = (io: any, participants: Player[], handSize: number) => {
	...
	var id = createId();
	var newDeck: Card[] = shuffleDeck(generateDeck());
	...
	var round: Round = createRound(0, newDeck, newPlayers);
...
export const handleBet = (io: any, socket: Socket, bet: number) => {
	var match: Match | undefined = findMatchBySocketId(socket.id);
	if (match) {
		var player = findPlayerById(socket.id);
...
export const playCard = (io: any, socket: Socket, card: Card) => {
	...
	if (match && player) {
		...
		if (isTurn && cardExists) {
			...
			var circuit = match.round.circuit;
...
export const handleTurn = (io: any, match: Match) => {
	...
	let playerTurn: number | undefined = match.round.circuit.turnToPlay;
...
export const processCircuit = (io: any, match: Match) => {
	...
	let leader: Player | undefined = match.round.circuit.currentLeader;
...
export const updateCardsRequested = (socket: Socket) => {
	var match: Match | undefined = findMatchBySocketId(socket.id);
	let player: Player | undefined = findPlayerById(socket.id);
	if (match && player) {
...
export const rematchRequested = (io: any, socket: Socket) => {
	let match : Match | undefined = findMatchBySocketId(socket.id);
	if (match) {
		match.rematch += 1;
